...
const UploadPopIt: React.FC = () => {
  const { popIts, addArquivo, updatePopIt, getArquivosByPopIt } = usePopItStore();
  const [selectedPopIt, setSelectedPopIt] = useState('');
  ...
  const handleUpload = async () => {
    if (!selectedPopIt || !file) {
      alert('Selecione um POP/IT e um arquivo');
      return;
    }

    const popIt = popIts.find(p => p.id === selectedPopIt);
    if (!popIt) return;

    const arquivosExistentes = getArquivosByPopIt(selectedPopIt);
    const novaVersao = arquivosExistentes.length + 1;

    // Gerar nome do arquivo com versão
    const extensao = file.name.split('.').pop();
    const nomeBase = popIt.titulo.toLowerCase().replace(/[^a-z0-9]+/g, '_');
    const nomeArquivo = `${nomeBase}_v${novaVersao}.${extensao}`;

    const novoArquivo = {
      id: Date.now().toString(),
      popItId: selectedPopIt,
      versao: novaVersao,
      nomeArquivo,
      dataUpload: new Date(),
      ...
      <div className="card max-w-2xl">
        <div className="space-y-6">
          <div className="form-group">
            <label htmlFor="popIt" className="form-label">Selecione o POP/IT</label>
            <select
              id="popIt"
              value={selectedPopIt}
              onChange={(e) => setSelectedPopIt(e.target.value)}
              className="form-select"
              required
            >
              <option value="">Selecione um POP/IT</option>
              {popIts.map((popIt) => (
                <option key={popIt.id} value={popIt.id}>
                  {popIt.titulo} - {popIt.setor}
                </option>
              ))}
            </select>
          </div>
