/*
  # Database Schema for Quality Management System

  1. Tables
    - Toners and returns tracking
    - Warranty management
    - Document management (POP/IT, BPMN)
    - Non-conformity management
    - Analysis tools (Ishikawa, Pareto)
    - Audit management
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'garantia_itens' 
    AND policyname = 'Allow read for authenticated users'
  ) THEN
    CREATE POLICY "Allow read for authenticated users"
      ON garantia_itens
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'pop_it_arquivos' 
    AND policyname = 'Allow read for authenticated users'
  ) THEN
    CREATE POLICY "Allow read for authenticated users"
      ON pop_it_arquivos
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'nc_verificacoes' 
    AND policyname = 'Allow read for authenticated users'
  ) THEN
    CREATE POLICY "Allow read for authenticated users"
      ON nc_verificacoes
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'ishikawa_analises' 
    AND policyname = 'Allow read for authenticated users'
  ) THEN
    CREATE POLICY "Allow read for authenticated users"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'ishikawa_causas' 
    AND policyname = 'Allow read for authenticated users'
  ) THEN
    CREATE POLICY "Allow read for authenticated users"
      ON ishikawa_causas
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'pareto_analises' 
    AND policyname = 'Allow read for authenticated users'
  ) THEN
    CREATE POLICY "Allow read for authenticated users"
      ON pareto_analises
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'formularios_auditoria' 
    AND policyname = 'Allow read for authenticated users'
  ) THEN
    CREATE POLICY "Allow read for authenticated users"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'itens_auditaveis' 
    AND policyname = 'Allow read for authenticated users'
  ) THEN
    CREATE POLICY "Allow read for authenticated users"
      ON itens_auditaveis
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'respostas_auditoria' 
    AND policyname = 'Allow read for authenticated users'
  ) THEN
    CREATE POLICY "Allow read for authenticated users"
